...
class Friday : Fragment() {
    val WEEKDAY : String = "금"

    private lateinit var rv_friday : RecyclerView
    private lateinit var rvManager: RecyclerView.LayoutManager
    val webtoonList : ArrayList<ListItem> = arrayListOf()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        var rootView = inflater.inflate(R.layout.fragment_friday, container, false)

        //초기화
        rv_friday = rootView.findViewById(R.id.rv_friday)
    ...
    private fun getListOfWebtoonsByDay() {
        doAsync {
            val document = Jsoup.connect("https://comic.naver.com/webtoon/weekdayList.nhn?week=fri").get()
