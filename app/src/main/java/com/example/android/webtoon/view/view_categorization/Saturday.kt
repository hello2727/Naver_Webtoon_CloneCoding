...
class Saturday : Fragment() {
    val WEEKDAY : String = "토"

    private lateinit var rv_saturday : RecyclerView
    private lateinit var rvManager: RecyclerView.LayoutManager
    val webtoonList : ArrayList<ListItem> = arrayListOf()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        var rootView = inflater.inflate(R.layout.fragment_saturday, container, false)

        //초기화
        rv_saturday = rootView.findViewById(R.id.rv_saturday)
    ...
    private fun getListOfWebtoonsByDay() {
        doAsync {
            val document = Jsoup.connect("https://comic.naver.com/webtoon/weekdayList.nhn?week=sat").get()
